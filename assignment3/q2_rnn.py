...
def pad_sequences(data, max_length):
    ...
    for sentence, labels in data:
        ...
        if len(sentence)>max_length: ret.append((sentence[:max_length],labels[:max_length],
                                                 [True]*len(sentence[:max_length])))
        else:
            diff = max_length-len(sentence)
            mask = [True]*len(sentence)+[False]*diff
...
class RNNModel(NERModel):
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(params=embedding, ids=self.input_placeholder)
    ...
    def add_prediction_op(self):
        ...
        with tf.variable_scope('layer1'):
            U = tf.get_variable(shape=(Config.hidden_size,Config.n_classes),
                            initializer=tf.contrib.layers.xavier_initializer(),name='U')
            b2 = tf.get_variable(shape=(self.config.n_classes),initializer=tf.zeros_initializer(),name='b2')
            state = tf.zeros(shape=[tf.shape(x)[0],Config.hidden_size])
        ...
        with tf.variable_scope("RNN") as scope:
            for time_step in range(self.max_length):
                ...
                o_drop_t = tf.nn.dropout(o_t,keep_prob=(1-dropout_rate))
        ...
        preds = tf.stack(values=preds, axis=0)
    ...
    def add_loss_op(self, preds):
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=masked_labels,logits=masked_preds)
