...
def make_windowed_data(data, start, end, window_size = 1):
    ...
    for sentence, labels in data:
        ...
        idx = 0
        for i in range(window_size, sent_len+window_size):
            feature = []
            for j in range(i-window_size,i+window_size+1):
                feature.extend(sentence[j])

            windowed_data.append((feature, labels[idx]))
            idx+=1
    #print(windowed_data)
...
class WindowModel(NERModel):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None, dropout=1):
        ...
        if labels_batch is not None:
        	feed_dict = {self.input_placeholder:inputs_batch, 
        	                  self.labels_placeholder:labels_batch, self.dropout_placeholder:dropout}
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(shape=(self.config.n_window_features*self.config.embed_size,
                                   self.config.hidden_size),name='weight1',
        		                   initializer=tf.contrib.layers.xavier_initializer())
        b1 = tf.get_variable(shape=(self.config.hidden_size,), name='bias1', initializer=tf.zeros_initializer())
        U = tf.get_variable(shape=(self.config.hidden_size,self.config.n_classes), name='weight2',
        		                initializer=tf.contrib.layers.xavier_initializer())
        b2 = tf.get_variable(shape=(self.config.n_classes,), name='bias2', initializer=tf.zeros_initializer())
