...
class GRUCell(tf.nn.rnn_cell.RNNCell):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        with tf.variable_scope(scope):
            ...
            W_r = tf.get_variable(shape=(self.state_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='W_r')
            U_r = tf.get_variable(shape=(self.input_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='U_r')
            b_r = tf.get_variable(shape=(self.state_size,),initializer=tf.zeros_initializer(),name='b_r')
            W_o = tf.get_variable(shape=(self.state_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='W_o')
            U_o = tf.get_variable(shape=(self.input_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='U_o')
            b_o = tf.get_variable(shape=(self.state_size,),initializer=tf.zeros_initializer(),name='b_o')
            W_z = tf.get_variable(shape=(self.state_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='W_z')
            U_z = tf.get_variable(shape=(self.input_size,self.state_size),
                                  initializer=tf.contrib.layers.xavier_initializer(),name='U_z')
            b_z = tf.get_variable(shape=(self.state_size,),initializer=tf.zeros_initializer(),name='b_z')

            z = tf.sigmoid(tf.matmul(inputs,U_z)+tf.matmul(state,W_z)+b_z)
            r = tf.sigmoid(tf.matmul(inputs,U_r)+tf.matmul(state,W_r)+b_r)
            o = tf.tanh(tf.matmul(inputs,U_o)+(r*tf.matmul(state,W_o)+b_o))
