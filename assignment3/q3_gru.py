...
class SequencePredictor(Model):
    ...
    def add_training_op(self, loss):
        ...
        variable = [variable for (grad,var) in grads_and_vars]

        if self.compute_gradients :
            grads = tf.clip_by_global_norm(grads, clip_norm=self.config.max_grad_norm)[0]

        grads_and_vars = [(grads[i],variable[i]) for i in range(len(grads_and_vars))]
