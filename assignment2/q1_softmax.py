...
def softmax(x):
    ...
    xc = tf.subtract(x, tf.reduce_max(x,1,keep_dims=True))
    e_x = tf.exp(xc)
    out = tf.div(e_x,tf.reduce_sum(e_x, axis=1,keep_dims=True))
...
def cross_entropy_loss(y, yhat):
    ...
    #y = tf.to_float(y)
    cross_entropy = tf.to_float(y)*tf.log(yhat)
    out = tf.negative(tf.reduce_sum(cross_entropy))
