...
class ParserModel(Model):
    ...
    def add_prediction_op(self):
        ...
        W = tf.get_variable(name='W1',
        	                initializer=initializer(shape=(self.config.n_features*self.config.embed_size, 
                            self.config.hidden_size)))
        U = tf.Variable(initializer(shape=(self.config.hidden_size, 
                        self.config.n_classes)))
        b1 = tf.Variable(tf.zeros(shape=(self.config.hidden_size,)))
        b2 = tf.Variable(tf.zeros(shape=(self.config.n_classes)))
