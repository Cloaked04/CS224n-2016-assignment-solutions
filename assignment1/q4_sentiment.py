...
def getSentenceFeatures(tokens, wordVectors, sentence):
    ...
    sentIndices = tokens[sentence] 
    sentVector = np.sum(wordVectors[sentIndices,:], axis=0)/len(sentIndices)
...
def getRegularizationValues():
    ...
    values = [0.1, 1.0, 0.001, 2.0, 2.5, 0.8, 0.7, 0.32, 5.0, 3.0, 0.58, 3.05]
...
def chooseBestModel(results):
    ...
    for index,model in enumerate(results):
    	if(index == 0):
    		bestResult = model
    	else if(model["reg"]>bestResult["reg"]):
    		bestResult = model
